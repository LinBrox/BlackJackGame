...
public class BlackjackGame {
	...
	public void initializeGame() {
		...
		System.out.println("	-If the player total equals the dealer total, it is a “Draw” and the hand ends.");
		System.out.println(
		...
		if (dealer.isBlackjack()) {
			...
			for (int i = 0; i < users; i++) {
				if (players[i].getTotal() == 21) {
					System.out.println(players[i].getName() + " draws");
					players[i].push();
				} else {
	...
	public void hitOrStand() {
		...
		for (int i = 0; i < users; i++) {
			...
			do {
				do {
					System.out.print(players[i].getName() + " (D)eal or (H)old? ");
					command = ki.next();
			...
			} while (c != 'H' && players[i].getTotal() <= 21);
			if (players[i].getTotal() > 21) {
	...
	// This code calculates all possible winning scenarios
	public void settleScore() {
		...
		for (int i = 0; i < users; i++) {
			if (players[i].getTotal() > 21) {
				...
				System.out.println(players[i].getName() + " has drawn");
				players[i].push();
	...
	public void endGame() {
		...
		for (int i = 0; i < users; i++) {
			...
			System.out.println(players[i].getName() + " has ended the game.");
